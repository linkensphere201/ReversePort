...
suite('Reverse Proxy Extension Integration Tests', () => {
    ...
    test('start command should show error when ssh does not exist', async () => {
        ...
        const originalSshPath = config.get('sshPath', 'ssh');
        let capturedError = '';
        ...
        try {
            ...
            assert.ok(capturedError.includes('SSH command is unavailable'), `Expected SSH unavailable error message, got: ${capturedError}`);
        }
        finally {
